...
def prepare(output, case, actions):
    ...
    worker = worker.replace(
        marker,
        '        request["alternative_contract"] = "Pointwise min-marginal events; not a coherent route"\n'
        '        if args.policy == "active":\n'
        '            request["reason"] = "Observable alternative events or unsupported frontier; heuristic ranking"\n'
        + marker,
    )
    ...
    contract = {
        ...
        "query_limitation": "Current active heuristic uses pointwise alternatives, not coherent full routes",
        "selection": "Fixed existing guide; no thresholds adjusted; failures do not erase scored rows",
    }
