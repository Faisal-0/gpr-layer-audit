def pick_seeded_packet(
    ...
    *,
    ...
    proposal = np.where(proposal >= 0, emissions[_rows, np.maximum(proposal, 0)], -1)
    alternate = np.where(alternate >= 0, emissions[_rows, np.maximum(alternate, 0)], -1)
    ...
    return SeedConditionedPath(
        ...
        provenance={
            ...
            "ambiguity": "Exact competing emitted peaks, including same-sign splits",
