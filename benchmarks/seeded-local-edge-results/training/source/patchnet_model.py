...
def native_candidates(data, valid, surface):
    ...
    if data.ndim != 2 or valid.shape != data.shape or valid.dtype != np.bool_:
        raise ValueError("Radar and boolean validity must share [trace, sample] coordinates")
    supported = valid & np.isfinite(data)
    selected = np.zeros(data.shape, bool)
    for row, trace in enumerate(data):
        # A masked neighbor cannot create/remove a measured extremum. Splitting
        # at invalid samples also prevents treating gap borders as observations.
        boundaries = np.flatnonzero(np.diff(np.r_[False, supported[row], False]))
        extrema = []
        for start, stop in boundaries.reshape(-1, 2):
            if stop - start >= 3:
                segment = trace[start:stop]
                extrema.extend(start + find_peaks(segment)[0])
                extrema.extend(start + find_peaks(-segment)[0])
    ...
    return selected & supported & (data != 0)
