...
def summarize(folder):
    ...
    for order in (2, 3):
        ...
        for axis, (arm, run) in zip(axes, runs.items(), strict=True):
            ...
            axis.set_title(
                f"{first['case']} {'base' if order == 2 else 'subbase'} / {arm}: "
    ...
    regresses = any(
        values["pairwise"]["accepted_wrong"] > values["classical"]["accepted_wrong"]
        or values["pairwise"]["accepted_correct"] < values["classical"]["accepted_correct"]
        for values in result["layers"].values()
    )
    result["decision"] = (
        "Reject: added wrong acceptances or lost correct coverage"
        if regresses
        else "Requires independent gate and coverage review before any promotion"
    )
